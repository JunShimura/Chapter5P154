...
//デバイスがロスとしてしまうので注意
	...
	for (UINT i = 0; i < swcDesc.BufferCount; ++i) {
		...
		_dev->CreateRenderTargetView(_backBuffers[i], &rtvDesc, handle);//RTVをディスクリプターヒープに作成
	...
	D3D12_INPUT_ELEMENT_DESC inputLayout[] =
	{
		...
		{// uv（追加）
			"TEXCOORD",
			0,
			DXGI_FORMAT_R32G32_FLOAT,	// float二つ分
			0,
			D3D12_APPEND_ALIGNED_ELEMENT,
			D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,
			0
...
#pragma region Chapter5.8.3
	...
	rootparam.ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;// ピクセルシェーダーから見える
	rootparam.ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;// ディスクリプタレンジのアドレス
	rootparam.DescriptorTable.pDescriptorRanges = &descTblRange;// ディスクリプタレンジ数
...
#pragma endregion
	...
	//result = LoadFromWICFile(L"img/black2white.png", WIC_FLAGS_NONE, &metadata, scratchImg);
	//result = LoadFromWICFile(L"img/black.png", WIC_FLAGS_NONE, &metadata, scratchImg);
